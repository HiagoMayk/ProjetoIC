...
public class Rtest extends Roteamento
{
	public Rtest(Graph grafo, Processor mapeamento[][], int linhas, int colunas)
	{
	...
	public void executeByStep()	
	{
		while(count < grafo.getEdges().size())
		{
			...
			for(int i = 0; i < linhas; i++)
			{
				for(int j = 0; j < colunas; j++)
				{
					//--->>>>>Devemos verificar para todos oelementos e enviar todos os que podem ser enviados
					for(int b = 0; b < roteadores[i][j].getBufferOut().size(); b++)
					{
						//2 é a quantidade maxima de pacotes que podem ser mandados em paralelo,
						//pois o XY_YX sempre tentam as mesmas direções.
						int t = 0;
						while(t < 2)
						{
							...
							Pacote pacote = roteadores[i][j].getBufferOut().get(0);							
							if(pacote.getCurrentCoordinate().getColumn() < pacote.getCoordinateDestination().getColumn())
				            {
		                        ...
		                        if(pacote.getCurrentCoordinate().getLine() == 0)
		                        {
		                                   ...
		                                   acumulator.returnEnlace(mapeamento[pacote.getCurrentCoordinate().getLine()][pacote.getCurrentCoordinate().getColumn()].getId(), mapeamento[pacote.getCurrentCoordinate().getLine()+1][pacote.getCurrentCoordinate().getColumn()].getId()).getAcessos())))
		                                {
		                                		...
		                                		if(acumulator.returnEnlace(mapeamento[pacote.getCurrentCoordinate().getLine()][pacote.getCurrentCoordinate().getColumn()].getId(), mapeamento[pacote.getCurrentCoordinate().getLine()][pacote.getCurrentCoordinate().getColumn()+1].getId()).getPacote() != null)
		                                		{
			                                        ...
			                                        roteadores[i][j].getBufferOut().get(0);
		                                		}	
		                                }
		                                else
		                                {
